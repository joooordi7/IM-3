document.addEventListener('DOMContentLoaded', () => {
    ...
    // Filter-Funktion für die Spielerliste nach Position
    const positionFilter = document.getElementById('position-filter');
    positionFilter.addEventListener('change', function() {
        const selectedPosition = this.value.toLowerCase(); // Ausgewählte Position
        playersTableBody.innerHTML = ''; // Tabelle zurücksetzen

        players
            .filter(player => selectedPosition === 'all' || player.position.toLowerCase() === selectedPosition)
            .forEach(player => {
                const playerRow = `
                    <tr>
                        <td>${player.name}</td>
